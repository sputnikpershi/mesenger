...
//            }
    ...
    func setup(with posts: [PostData], index: Int, account: Account?) {
        ...
        self.postImage.image =  UIImage(data: posts[index].postImage!)
...
//            let indexPost = account?.posts.firstIndex { post in
...
//            }
    ...
    extension LikeVCell: LikeDelegate {
        ...
        func likePost(_ isLike: inout Bool)   {
            print("unlike ")
//            guard let postArray = self.postArray else { return }
            self.posts = coreDataManager.posts
